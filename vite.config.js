...
export default defineConfig(async ({ command, mode }) => {
  ...
  if (command === "build") {
    console.log(
      "Sitemap generation: Fetching dynamic routes for production build..."
    );
    ...
    console.log(
      `Sitemap generation: Fetched ${dynamicSitemapRoutes.length} dynamic routes.`
    );
  }

  const allSitemapRoutes = [
    ...new Set([...staticRoutes, ...dynamicSitemapRoutes]),
  ];
